        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-8">
            {testimonials.map((test, index) => (
              <div
                key={test.id || index}
                className="flex flex-col justify-between p-2.5 md:p-6 rounded-2xl md:rounded-3xl bg-neutral-900/80 border border-neutral-850/50 hover:border-neutral-800 transition-premium group hover:-translate-y-1 hover:shadow-2xl"
              >
                <div>
                  {/* Rating Stars */}
                  <div className="flex items-center gap-0.5 md:gap-1.5 mb-2 md:mb-5">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`w-2 h-2 md:w-4 md:h-4 ${star <= test.rating
                  ...
                  <p className="text-[8px] md:text-sm text-gray-300 leading-relaxed font-light italic line-clamp-4 md:line-clamp-none">
                    &ldquo;{test.text}&rdquo;
                  </p>
                </div>

                <div className="flex items-center gap-1.5 md:gap-4 mt-3 md:mt-8 pt-2 md:pt-5 border-t border-neutral-900">
                  <Image
                    src={test.avatar || "/logo.png"}
                    alt={test.name}
                    width={40}
                    height={40}
                    className="w-5.5 h-5.5 md:w-10 md:h-10 rounded-full object-cover border border-neutral-800 group-hover:border-brand-red transition-premium flex-shrink-0"
                  />
                  <div className="min-w-0">
                    <h4 className="font-extrabold text-white text-[8px] md:text-sm tracking-wide truncate">{test.name}</h4>
                    <p className="text-[6.5px] md:text-[10px] font-bold text-brand-green uppercase tracking-wider truncate">{test.role}</p>
                  </div>
                </div>
              </div>
            ))}
